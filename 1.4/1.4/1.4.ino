...
void setup() {
  ...
  while (WiFi.status() != WL_CONNECTED && wifiReconnectStatic < 10) {
    //    Serial.print(".");
        delay(1000);
    wifiReconnectStatic++;
  }
...
void InputTask(void *pvParam) {
  ...
  // Timers
  bool timerOneOn = false;
  unsigned long timerOneStart = 0;
  while (1) {
    ...
    if (strcmp(inputMessage, "none") != 0) {
      ...
      if (strcmp(inputMessage, "inFive/1") == 0) {
        ...
        buttonCounter++;
        if (settings["timerTwo"] > 0) {
          if (timerOneOn == false) {
            SendMessageToOutputTask("output", "transOne", STOP_OUT);
            timerOneOn = true;
            timerOneStart = timeNow;
          }
        }
      }
    ...
    //------------------------------------------------------------------------------
    if (timerOneOn == true) {
      if (timeNow - timerOneStart > settings["timerOne"]) {
