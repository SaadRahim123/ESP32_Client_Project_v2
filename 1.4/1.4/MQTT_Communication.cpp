...
const char *mqtt_ID = "h7J9u3";
// MQTT Credentials
const char *mqtt_username = "remote2";
const char *mqtt_password = "password2";
const char *mqtt_client = "ESP32A02";  // This one?
// String clientId = "MQTTClient-";  // and use it here aswell? you can do that, makes it easier to create new
...
void CallbackTask(void *pvParam) {
  ...
  while (1) {
    ...
    if ((callback_data.dataArrives)) {
      ...
      Serial.print(" Data: ");
      Serial.println(payloadData);  // If this is "" then // maybe solves the issue but let just check one thing
      Serial.print("The payloadfunc is "); Serial.println(payloadFunc);
      // If topic is a set
...
void MQTTreconnect() {
  ...
  //clientId += 92;
  // mqtt_ID  // Like that? it will work.. give it a try ok how come it has to be a c_str ?dnt check that
  // Attempt to connect
